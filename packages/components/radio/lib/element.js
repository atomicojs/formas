...
import { useMemo, useHost, useRef, useEffect, useLayoutEffect, useState, useProp, h, options, template, css, c } from 'atomico';
...
function useDisabled(matches = "fieldset") {
  ...
  useMutationObserver(fieldset, (items) => items.filter((item) => item.attributeName == "disabled").map((el) => {
...
function useRender(callback, args) {
  ...
  useMemo(() => !options.ssr && fillHost(callback()).render(host2.current, host2.id), args);
...
var reflectEvent = (current, event, composed) => {
  ...
  const insetShadowRoot = path.slice(0, index).find((el) => el instanceof ShadowRoot);
...
const GenericTokens = css`@tokens "./tokens.yaml" (import: generic) (values: true);:host{font-size:var(--font-size);line-height:var(--font-line);--outline-color: var(--color-outline);--outline: var(--outline-width) var(--outline-style) var(--outline-color)}:host([disabled]){opacity:var(--opacity);pointer-events:none}:host([focused]:not([disabled])){--outline-color: var(--color-outline-on)}`;
const GenericStateTokens = css`@tokens "./tokens.yaml" (filter: state);`;
const NavigationTokens = css`@tokens "./tokens.yaml" (import: navigation);`;
const ButtonTokens = css`@tokens "./tokens.yaml" (import: button);`;
const DropdownTokens = css`@tokens "./tokens.yaml" (import: dropdown);`;
const CheckboxTokens = css`@tokens "./tokens.yaml" (import: checkbox);`;
const RadioTokens = css`@tokens "./tokens.yaml" (import: radio);`;
const SwitchTokens = css`@tokens "./tokens.yaml" (import: switch);`;
const InputTokens = css`@tokens "./tokens.yaml" (import: input);`;
...
icon.styles = [
  GenericTokens,
  css`:host{width:var(--width, var(--size-icon));color:var(--color, currentColor);display:inline-flex;align-items:center;justify-items:center}svg{width:100%;margin:auto}path{fill:currentColor}`
];
const Icon = c(icon);
...
function checkbox({ tabIndex }) {
  ...
  return /* @__PURE__ */ jsx("host", { shadowDom: true, children: /* @__PURE__ */ jsx(
    "button",
    {
      ...
      children: /* @__PURE__ */ jsx("div", { class: "checkbox-box", children: /* @__PURE__ */ jsx("div", { class: "checkbox-state", children: /* @__PURE__ */ jsx(Icon, { type: "check" }) }) })
    }
  ) });
}
checkbox.props = {
...
checkbox.styles = [
  GenericTokens,
  CheckboxTokens,
  css`:host{--box-width: .6;--box-height: var(--box-width);--state-unit: 100%;--state-width: 1;--state-height: var(--state-width);--state-radius: 0px;--state-transform: none;--state-transition: .25s;--background: var(--color-checkbox);--background-state: var(--color-state);--color: var(--color-content);--border-color: var(--color-border);color:var(--color);display:inline-block}:host([checked]){--background: var(--color-checkbox-on);--background-state: var(--color-state-on);--border-color: var(--color-border-on);--color: var(--color-content-on)}.checkbox{width:var(--size-height);height:var(--size-height);background:transparent;border:none;padding:0;cursor:pointer;display:flex;outline:none}.checkbox-box{width:calc(100% * var(--box-width));height:calc(100% * var(--box-height));margin:auto;border-radius:var(--radius);border:var(--border-width) var(--border-style) var(--border-color);background:var(--background);overflow:hidden;box-sizing:border-box;display:flex;outline:var(--outline)}.checkbox-state{width:calc(var(--state-unit) * var(--state-width));height:calc(var(--state-unit) * var(--state-height));border-radius:var(--state-radius);display:flex;align-items:center;justify-content:center;background:var(--background-state);margin:auto;transform:var(--state-transform);transition:var(--state-transition)}`
];
...
function component({ tabIndex, value }) {
  ...
  return /* @__PURE__ */ jsx("host", { shadowDom: true, children: /* @__PURE__ */ jsx(
    "button",
    {
      ...
      onfocus: () => setFocused(true),
      onblur: () => setFocused(false),
      children: /* @__PURE__ */ jsx("div", { class: "checkbox-box", children: /* @__PURE__ */ jsx("div", { class: "checkbox-state" }) })
    }
  ) });
}
component.props = {
...
component.styles = [
  Checkbox.styles,
  RadioTokens,
  css`:host{--state-width: .6;--radius: var(--radius-circle);--state-unit: calc(var(--size-height) * var(--box-height));--state-radius: var(--radius-circle)}`
];
